...
This function parses a city from the format of the HashCode2014 package and returns a `CityProblemInst` object.
"""
...
"""
    read_city_problem(city)

Generates a `CityProblemInst` for a given city.
"""
...
This computes the total distance that a solution travels.
"""
function total_distance(solution::Solution, city::CityProblemInst=read_city_problem())
    ...
    for itinerary in solution.itineraries
        for i in 1:(length(itinerary) - 1)
            if !(
                (itinerary[i], itinerary[i + 1]) in used ||
                (itinerary[i + 1], itinerary[i]) in used
            )
